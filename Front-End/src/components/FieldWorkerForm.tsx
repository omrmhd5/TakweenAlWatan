...
// Updated to only include the 5 specific districts
const districts = [
  'العزيزية',
  'المعابدة',
  'الشرائع',
...
export default function FieldWorkerForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    district: '',
    siteCounts: siteTypes.reduce((acc, site) => ({ ...acc, [site]: 0 }), {} as SiteCounts)
  });
  ...
  const totalSites = Object.values(formData.siteCounts).reduce((sum, count) => sum + count, 0);

  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-8">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-bold text-gray-900 mb-2">نموذج إدخال البيانات الميدانية</h2>
            ...
            {siteTypes.map((siteType, index) => (
              <div key={siteType} className="bg-gray-50 p-4 rounded-lg border">
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange(`site_${siteType}`, e.target.value)}
      ...
      {showConfirmation && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 max-w-md mx-4">
            <h3 className="text-xl font-bold text-gray-900 mb-4 text-center">تأكيد الحفظ</h3>
            <div className="space-y-2 mb-6 text-sm">
              <p><strong>التاريخ:</strong> {formData.date}</p>
              <p><strong>الحي:</strong> {formData.district}</p>
              <p><strong>إجمالي المواقع:</strong> {totalSites}</p>
            </div>
